...
export class SessionComponent implements OnInit {
      ...
      .then((result) => {
        ...
        if (credential) {
          ...
          if (credential && 'accessToken' in credential) {
            ...
            this.httpClient.get(url).subscribe((userInfo: any) => {
              // Obtener el nombre y el apellido del objeto userInfo
              const firstName = userInfo.given_name;
              const lastName = userInfo.family_name;
              
              this.verificar(undefined,undefined,userInfo.email);
  ...
  //doy el correo , me da el pass y contra
  unUsuario: Login = new Login();
  verificar(username?: string, password?: string, correo?:string) {
    if(username != undefined && password != undefined){
      this.unUsuario.password = password;
      this.unUsuario.username = username;
      console.log(
        '🔥 > Por base de datos > usuario:',
        this.unUsuario.username + ' / ' + this.encryp.decrypt(this.unUsuario.password)
      );
    ...
    if(correo != undefined){
      this.clienteService.byCorreo(correo,this.almacen.getToken()).subscribe(
        (data : Cliente) => {
          this.unUsuario.username = data.idUserCliente.username;
          console.log(
            '🔥 > Por base de datos > usuario:',
            this.unUsuario.username
          );
          this.login.ingresarGoogle(this.unUsuario).subscribe(
            async (data) => {
              const token = (data as { token: string }).token;
              this.almacen.setToken(token);
              this.dataService.updateVariable(true);
